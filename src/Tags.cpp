...
uint32_t utf8_next(const char *&ptr, const char *end) {
  ...
  if (first_byte < 0x80) {
    ...
    if (ptr + 1 >= end)
      return 0;
    code_point =
        ((first_byte & 0x0F) << 12) | ((ptr[0] & 0x3F) << 6) | (ptr[1] & 0x3F);
    ptr += 2;
...
void Tags::Init() {
  ...
  it++) { Tags::info& info=*it; cout << " { " << info.sep << " - " << info.entry
  << " - " << info.pos << " } ";
  }
  */
...
void Tags::add_parameter_block(std::vector<Tags::Entity>::size_type i,
  ...
  for (std::vector<EntityIndexPair>::size_type it = 0;
       it < parameter_list.size(); it++) {
    group.clear();
    for (std::vector<Tags::Entity>::size_type k = parameter_list[it].first + 1;
         k < parameter_list[it].second; k++)
      group.push_back(tokens[k]);
              ...
              if (b) {
                ...
                */
                ...
                if (b & Tags::is_associated_last(index, index_end)) {
                  ...
                  Tags::create_tag(i, index, document, parameter_block_list[0]);
              ...
              if (b) {
                ...
                while (b) {
                  ...
                  if (b & Tags::is_associated_last(index, index_end)) {
                    ...
                    for (std::vector<std::vector<EntityIndexPair> >::size_type
                             ind = 0;
                         ind < parameter_block_list.size(); ind++) {
                      add_parameter_block(i, document,
                                          parameter_block_list[ind]);
                    }
                    for (std::vector<EntityIndexPair>::size_type ind = 0;
                         ind < argument_list.size(); ind++) {
...
Tags::GetParameters(const Tags::Entity &tag) {
...
Tags::GetArguments(const Tags::Entity &tag) {
...
void Tags::display_parameters(
    std::vector<std::vector<std::vector<std::string> > > p) {
  for (std::vector<std::vector<std::vector<std::string> > >::iterator it_b =
           p.begin();
       it_b != p.end(); ++it_b) {
    std::cout << "block" << std::endl;

    // Correct: *it_b is a vector<vector<string> >, so b should be a reference to
    // that
    ...
    for (std::vector<std::vector<std::string> >::iterator it_parameters =
      ...
      // Correct: *it_parameters is a vector<string>, so parameters should be a
      // reference to that
      std::vector<std::string> &parameters = *it_parameters;

      for (std::vector<std::string>::iterator it_s = parameters.begin();
           it_s != parameters.end(); ++it_s) {
        std::string s = *it_s;
        std::cout << " - " << s;
      }
      std::cout << std::endl;
...
void Tags::display_arguments(vector<vector<std::string> > arguments) {
  for (vector<vector<std::string> >::iterator it_argument = arguments.begin();
       it_argument != arguments.end(); ++it_argument) {
    std::vector<std::string> &argument = *it_argument;
    cout << "argument" << endl;
    cout << "   ";
    for (vector<std::string>::iterator it_s = argument.begin();
         it_s != argument.end(); ++it_s) {
      std::string s = *it_s;
      cout << " - " << s;
    }
    cout << endl;
        ...
        if (pa.tagNumber == nParameterBlocks) {
          ...
          if (!pa.entityList.empty()) {
            for (std::vector<Tags::Entity>::size_type ind_pa = 0;
              ...
              // cout << "parameter? (ngroup)" << group.tagNumber << endl;
              if (!group.entityList.empty()) {
                for (std::vector<Tags::Entity>::size_type l = 0;
                     l < group.entityList.size(); l++) {
                  if (group.entityList[l].tagNumber == nTag) {
                    std::vector<Tags::Entity> &elem = group.entityList;
         ...
         // DisplayEntity(elist);
          if (!pa.entityList.empty()) {
            for (std::vector<Tags::Entity>::size_type m = 0;
              ...
              Tags::Entity &gra = pa.entityList[m];
              if (gra.tagNumber == nTag) {
                tagListList[level].push_back(Position(&(pa.entityList), m));
                // std::vector<Tags::Entity> elist;
                // elist.push_back(gra);
                // DisplayEntity(elist);
              }
  ...
  for (std::vector<std::vector<Tags::Position> >::size_type v = 0; v <=level;
    ...
    for (std::vector<Position>::size_type ip = 0; ip < pList.size(); ip++) {
      ...
      Tags::Entity &tag = (*(p.list))[p.index];
        ...
        std::vector<std::vector<std::string> > result =
            call_lua_function(L, command_name, params, args);

        // Print the result
        /*
            std::cout << "Result:" << std::endl;
            for (std::vector<std::vector<std::string> >::size_type i = 0; i <
           result.size(); ++i) { for (std::vector<std::string>::size_type j = 0;
           j < result[i].size(); ++j) { std::cout << result[i][j] << " ";
              }
              std::cout << std::endl;
            }
        */
        ...
        for (std::vector<std::vector<std::string> >::size_type i = 0;
             i < result.size(); ++i) {
          for (std::vector<std::string>::size_type j = 0; j < result[i].size();
               ++j) {
            res += result[i][j];
          }
        }
  ...
  while (it != text.end()) {
    ...
    try {
      ...
      if (first_byte < 0x80) {
        ...
        if (it + 2 >= text.end())
          throw utf8::exception();
        code_point = ((first_byte & 0x0F) << 12) | ((it[1] & 0x3F) << 6) |
                     (it[2] & 0x3F);
        byte_length = 3;
      } else if ((first_byte & 0xF8) == 0xF0) {
        // 4-byte sequence
        if (it + 3 >= text.end())
          throw utf8::exception();
        code_point = ((first_byte & 0x07) << 18) | ((it[1] & 0x3F) << 12) |
    ...
    if (code_point == '\\') {
      ...
      if (next_it != text.end()) {
        ...
        if (next_cp == '[' || next_cp == ']' || next_cp == '|') {
          // Found escaped delimiter - add the delimiter itself (without
          // backslash)
    ...
    // Check for delimiters
      ...
      Tags::document.push_back(Tags::Entity(
          CharToTagNumber(code_point), delimiter, std::vector<Tags::Entity>()));
      it += byte_length;
    } else {
...
void Tags::ScanFile(std::string fileName) {
  ...
  while (i < textLength) {
    ...
    if (IsNotDelim(c)) {
      ...
      std::string s = sb;
      document.push_back(Tags::Entity(nString, s, std::vector<Tags::Entity>()));
      --i;
